...
const PacketPage: React.FC = () => {
  ...
  const handleExport = () => {
    const packetTransactions = transactions.filter(t => t.entryType === 'packet');
    if (packetTransactions.length === 0) {
      customAlertWarning('No Packet transactions to export!');
      return;
    }
    exportTransactionsToExcel(packetTransactions, `${project?.name || 'Project'}-Packet`);
  };
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const result = await importTransactionsFromExcel(file, id || '', 'packet');

      if (result.success && result.transactions.length > 0) {
        for (const t of result.transactions) {
          await addTransaction(t);
        }
        customAlertSuccess(`Successfully imported ${result.transactions.length} Packet transaction(s)!`);
        refresh();
      }
      ...
      if (result.transactions.length === 0 && result.errors.length === 0) {
        customAlertWarning('No Packet transactions found in the file.');
      }
    } catch (error) {
  ...
  // Redirect if project doesn't support Packet
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
          <div className="flex-1 max-w-md">
            <input
              ...
              placeholder="Search numbers (e.g., 1234, 1*, *5, **34)"
      ...
      {activeTab === 'entries' ? (
        <div className="w-full">
          ...
          {/* Number Grid - Full Width with proper padding - Uses PacketGrid component */}
