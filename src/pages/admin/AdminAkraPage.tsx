...
const AdminAkraPage: React.FC = () => {
  ...
  const loadEntries = useCallback(async (force = false, showLoader = true) => {
    ...
    try {
      const [freshEntries, freshDeductions] = await Promise.all([
        db.getAllEntriesByType('akra', true),
        db.getAdminDeductionsByType('akra'),
      ]);
  ...
  const handleDelete = async () => {
    ...
    setCachedData(akraCacheConfig, updatedEntries);
    ...
    try {
      ...
      setCachedData(akraCacheConfig, entries);
  ...
  const handleDeleteDeduction = async (deduction: DeductionRecord) => {
    ...
    setDeductions(prev => {
      ...
      setCachedData(akraDeductionsCacheConfig, updated);
      return updated;
    });
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 py-8">
        ...
        {loadingEntries ? (
          <div className="p-6 space-y-4">
            {[...Array(10)].map((_, idx) => (
              <div key={idx} className="animate-pulse bg-gray-200 dark:bg-gray-700 h-16 w-full rounded" />
            ))}
          </div>
        ) : viewMode === 'aggregated' ? (
...
export default React.memo(AdminAkraPage);
