...
const AdminOpenPage: React.FC = () => {
  ...
  const loadEntries = useCallback(async (force = false, showLoader = true) => {
    ...
    try {
      const [freshEntries, freshDeductions] = await Promise.all([
        db.getAllEntriesByType('open', true),
        db.getAdminDeductionsByType('open'),
      ]);
  ...
  const handleDelete = async () => {
    ...
    setCachedData(openCacheConfig, updatedEntries);
    ...
    try {
      ...
      setCachedData(openCacheConfig, entries);
  ...
  const handleResetAll = async () => {
    ...
    const result = await confirm(
      `Are you sure you want to RESET the Admin View for Open entries?\n\nThis will PERMANENTLY DELETE:\n• All Open transactions (0)\n• All admin deductions for Open entries\n\n⚠️ This will only affect Open entries on this admin page.\n⚠️ Other entry types (Ring, Akra, Packet) will NOT be affected.`,
      { type: 'danger', title: '🔄 Reset Admin View - Open Entries Only' }
    );
    ...
    setResetting(true);
    try {
      const result = await db.deleteAllAdminDeductionsByType('open');
      await showSuccess('Success', `Reset admin view: Deleted ${result.deletedCount} Open entries and all associated deductions.`);
  ...
  const handleDeleteDeduction = async (deduction: DeductionRecord) => {
    ...
    setDeductions(prev => {
      ...
      setCachedData(openDeductionsCacheConfig, updated);
      return updated;
    });
  ...
  const handleEdit = async (updatedTransaction: any) => {
    ...
    try {
      ...
      setStats(prev => ({
        ...
        totalPkr: prev.totalPkr - (newTotal - oldTotal),
      }));
      setCachedData(openCacheConfig, entries);
...
export default React.memo(AdminOpenPage);
